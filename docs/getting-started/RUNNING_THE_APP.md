...
How to run KMPPTTDynamics on every target. Each section explains what to do, why it works, and how to confirm the app actually launched.
...
What the app shows when it runs correctly: the PTT (Pereira Tech Talks) vertical logo, a **Create meetup** button, a **Join with code** field, and two empty sections labelled *Live meetups* / *Past meetups*. If `.env` is missing, you instead see a friendly "Supabase is not configured" placeholder pointing at this guide.

The same UI is generated by the same `commonMain` Compose tree on every target — seeing it on Android, iOS, Desktop, and Web confirms the shared code is wired correctly across all five platform entry points.
...
adb shell monkey -p com.xergioalex.kmppttdynamics -c android.intent.category.LAUNCHER 1
```

### What you should see

The PTT Dynamics home screen rendered on the emulator: PTT logo, **Create meetup** button, **Join with code** field, empty *Live meetups* / *Past meetups* sections.

### Verify

```bash
adb logcat | grep -i kmppttdynamics
```
...
The app installed and launched on your phone.
...
PTT Dynamics rendered inside the simulator window.
...
A native desktop window titled *PTT Dynamics* with the home screen rendered.
...
KMPPTTDynamics ships two web targets:
...
PTT Dynamics rendered in the browser tab (Wasm or JS).
