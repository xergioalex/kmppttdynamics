...
Real problems hit while setting KMPPTTDynamics up on macOS, with the exact fix for each. If you're hitting something not listed here, open an issue.
...
The KMP template now appears under `New Project`. (KMPPTTDynamics is already a working KMP project, so you don't need this just to develop here — but the plugin also adds run configurations for iOS targets, which is useful.)
...
**Cause**: Gradle/Kotlin doesn't recognize the Java version it's running on. Brew's default `temurin` cask installs the latest JDK (Java 26 at the time of writing), which is too new for the Gradle/Kotlin tooling KMPPTTDynamics uses.
...
**Symptom**: Android Studio opens KMPPTTDynamics and Gradle sync fails with dependency-resolution or plugin-not-found errors.
...
2. In `iosApp.xcodeproj` → **iosApp** target → **Signing & Capabilities**:
   ...
   - Make sure **Bundle Identifier** is unique (Apple's free signing requires globally unique IDs). If you're forking, change it from `com.xergioalex.kmppttdynamics` to your own — see [`../FORK_CUSTOMIZATION.md`](../FORK_CUSTOMIZATION.md).
