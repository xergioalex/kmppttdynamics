...
KMPStarter is a **Kotlin Multiplatform** template, not a Flutter project. This doc exists because both stacks target the same platforms (Android, iOS, Web, Desktop), and developers evaluating KMP often want to also try Flutter side by side.
...
KMPStarter exists because the KMP path has more upfront friction; documenting that friction (this `docs/getting-started/` folder) is part of making the choice fair.
